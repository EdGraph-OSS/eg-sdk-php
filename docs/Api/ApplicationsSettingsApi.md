...
```php
getClientSettingsAsync($tenantId, $clientId): \EdGraph\PlatformClient\Model\TenantApiTenantV1GetAppSettingsResponse
```
...
try {
    $result = $apiInstance->getClientSettingsAsync($tenantId, $clientId);
    print_r($result);
} catch (Exception $e) {
